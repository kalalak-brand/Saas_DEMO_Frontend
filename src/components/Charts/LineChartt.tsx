...
// Read primary color from CSS variable (set by ThemeProvider)
const getThemeColor = () => getComputedStyle(document.documentElement).getPropertyValue('--color-primary').trim() || '#1d4d4f';
  ...
  if (active && payload && payload.length) {
    return (
      <div
        ...
        style={{ backgroundColor: getThemeColor(), color: 'white', fontSize: '0.875rem' }}
      >
...
export const LineChartt: React.FC<DynamicLineChartProps> = ({ data, title, ytdValue, timeLabel }) => {
  return (
    <div
      ...
      style={{ backgroundColor: COLOR_BACKGROUND, color: getThemeColor(), margin: '20px' }}
    >
      ...
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={data} margin={{ top: 20, right: 20, left: 20, bottom: 20 }}>
          ...
          <Line
            ...
            strokeWidth={3}
            dot={{ r: 4, fill: getThemeColor() }}
            activeDot={{ r: 6, stroke: getThemeColor(), strokeWidth: 2 }}
          />
        </LineChart>
      </ResponsiveContainer>
    </div>
